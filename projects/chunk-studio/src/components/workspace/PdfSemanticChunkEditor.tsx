...
export default function PdfSemanticChunkEditor({
  ...
  useEffect(() => {
    ...
    if (!viewport) return;
    const onScroll = () => state.updateCurrentPageFromViewport(viewport);
    ...
    return (
      <section className="workspace-shell" style={{ justifyContent: "center", alignItems: "center" }}>
        <div style={{ textAlign: "center", display: "grid", gap: 10 }}>
          ...
          <button type="button" onClick={() => fileInputRef.current?.click()} style={floatingButton}>
            PDF 업로드
          </button>
          <input
            ref={fileInputRef}
          ...
          >
            {state.canPreviewPdf && !state.pdfUnavailable && state.pdfAvailabilityChecked ? (
              <PdfPreviewClient
                ...
                onLoadError={() => {
                  state.setFailedPdfJobId(selectedJob.id);
