...
export async function runLegacyProblemInterviewFallbackPipeline(params: {
  ...
  readonly stateJsonRef: MutableRefObject<{ promptTimeline?: unknown; problemInterview?: unknown; singleChatOrchestrationV1?: unknown }>;
  ...
  const absorbPromptTrace = (raw: unknown) => {
    ...
    if (!tr) return;
    stateJsonRef.current = mergeRequirementsStateJson(stateJsonRef.current as any, {
      promptTimeline: appendIdeationBootstrapPromptTimeline((stateJsonRef.current as any).promptTimeline, tr),
    }) as any;
  };
    ...
    if (plan) {
      ...
      if (
        ...
        await persistRemote(withCalling, {}, {
          ...
          ...((stateJsonRef.current as any).promptTimeline ? { promptTimeline: (stateJsonRef.current as any).promptTimeline } : {}),
        });
  ...
  const nowIso = new Date().toISOString();
  const prevPi = ((stateJsonRef.current as any).problemInterview as ProblemInterviewState | null | undefined) ?? null;
  ...
  const replySlotFromUser =
    (lastUser as any)?.meta?.replyToSlotKey && String((lastUser as any).meta.replyToSlotKey).trim()
      ? String((lastUser as any).meta.replyToSlotKey).trim()
      : undefined;
  const replyTargetFromUser =
    (lastUser as any)?.meta?.replyTargetSpeakerId && String((lastUser as any).meta.replyTargetSpeakerId).trim()
      ? String((lastUser as any).meta.replyTargetSpeakerId).trim()
      : undefined;
  ...
  if (pid) {
    try {
      ...
      const orchBody = (stateJsonRef.current as any).singleChatOrchestrationV1;
      ...
      const ar = await credentialsIncludeFetch(REQUIREMENTS_IDEATION_HTTP.INTERVIEW_ANALYZE, {
        ...
        body: JSON.stringify({
          ...
          latestAiQuestion,
          currentSlotKey:
            typeof (latestAiTurn as any)?.meta?.problemInterviewLastSlot === "string" &&
            String((latestAiTurn as any).meta.problemInterviewLastSlot).trim()
              ? String((latestAiTurn as any).meta.problemInterviewLastSlot).trim()
              : undefined,
  ...
  if (outcome.kind === "parsed") {
    ...
    const lastSlotRaw = String((latestAiTurn as any)?.meta?.problemInterviewLastSlot ?? "").trim();
