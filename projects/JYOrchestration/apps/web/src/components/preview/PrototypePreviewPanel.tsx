...
} from "@/lib/prototype/prototypeInlineTemplateConstants";
import { PrototypeImplementationStagePanel } from "@/components/preview/PrototypeImplementationStagePanel";
import {
  usePrototypeImplementationStagePanel,
  type PrototypeImplementationStageHost,
...
import { mapExecutionSetupDtoToSourceGenerationRow } from "@/lib/prototype/executionSetupSourceGeneration";
...
import { buildPrototypeRunExecutionSyncPatch } from "@/lib/prototype/implementationPrototypeRunSync";
...
export function PrototypePreviewPanel({
  ...
  const [executionEnvRefreshInFlight, setEnvCheckInFlight] = useState(false);
  ...
  const showToast = useCallback((_msg: string, _displayMs?: number) => {}, []);
    ...
    (a: PrototypeChatAction) => {
      switch (a.intent) {
        ...
        case "REGENERATE_PLAN":
          ...
          return;
        case "MODIFY_REQUEST":
          showToast("수정 요청은 기획 단계 대화에서 진행해 주세요.");
          return;
        case "CANCEL_RUN":
  ...
  const implementationStageHost: PrototypeImplementationStageHost = {
    projectId,
    projectName,
    projectDescription,
    requirementsStateJson,
  ...
  const implementationStage = usePrototypeImplementationStagePanel(implementationStageHost);
  const { handleExecutionSetupChanged } = implementationStage;
  ...
  return (
    ...
    >
      ...
      `}</style>
      <PrototypeImplementationStagePanel host={implementationStageHost} stage={implementationStage} />
