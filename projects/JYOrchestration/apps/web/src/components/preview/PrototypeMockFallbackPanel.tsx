...
/** 생성 전 로컬 목업만 표시할 때 사용 (최종 산출물 아님). */
export function PrototypeMockFallbackPanel({
  projectName,
  projectDescription,
  ideationAssets,
  flowSteps,
  actors,
  recommendedTemplateOverride,
}: {
  readonly projectName?: string;
  readonly projectDescription?: string;
  readonly ideationAssets?: ReadonlyArray<PrototypeWorkspaceIdeationAsset>;
  readonly flowSteps?: ReadonlyArray<PrototypeWorkspaceFlowStep>;
  readonly actors?: ReadonlyArray<PrototypeWorkspaceActor>;
  /** 워크스페이스 분석 결과와 동기화할 시드 템플릿 */
  readonly recommendedTemplateOverride?: PrototypeTemplateType;
}) {
  ...
  useEffect(() => {
    const t = window.setTimeout(() => setTemplateId(recommendedTemplateOverride ?? rec.templateId), 0);
    return () => window.clearTimeout(t);
  }, [rec.templateId, recommendedTemplateOverride]);
  ...
  const frameWidth = device === "mobile" ? 340 : "100%";
  ...
  return (
    <div style={{ display: "grid", gap: 10 }}>
      <div style={{ fontSize: 11.5, fontWeight: 900, color: "#64748b" }}>
        생성 전 참고 화면 (예시) — Cursor로 생성한 프로토타입이 연결되면 이 영역은 대체됩니다.
      </div>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
        ...
        >
          시드: {PROTOTYPE_TEMPLATES.find((t) => t.id === rec.templateId)?.nameKo ?? rec.templateId} ({rec.score}%)
        </span>
      ...
      <div
        style={{
          width: typeof frameWidth === "number" ? frameWidth : undefined,
          maxWidth: "100%",
          borderRadius: 16,
      ...
      >
        ...
        {!hasFlowData ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: device === "mobile" ? "1fr" : "minmax(0, 180px) 1fr", minHeight: 320 }}>
            {device === "mobile" ? null : (
            ...
            <main style={{ padding: 12, display: "grid", gap: 12 }}>
              ...
              {selectedActor.kind === "system" ? (
                <section style={{ display: "grid", gap: 10 }}>
                  ...
                  <div style={{ display: "grid", gap: 8 }}>
                    {(myPrimary.length ? myPrimary : safeSteps).slice(0, 8).map((s, i) => (
                      <div
                        key={s.id}
                        style={{
                          display: "flex",
