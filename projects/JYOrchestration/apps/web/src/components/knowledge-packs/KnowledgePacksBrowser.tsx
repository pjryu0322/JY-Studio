...
export type KnowledgePacksBrowserProps = Readonly<{
  readonly variant?: "page" | "project-modal";
  readonly projectId?: string;
}>;

export function KnowledgePacksBrowser(p: KnowledgePacksBrowserProps) {
  const variant = p.variant ?? "page";
  const isProjectModal = variant === "project-modal";
  ...
  const urlSelectedId = useMemo(() => String(searchParams.get("id") ?? "").trim(), [searchParams]);
  const selectedId = isProjectModal ? localSelectedId : urlSelectedId;
  ...
  const selected = useMemo(() => {
    ...
    const fromFiltered = selectedId ? filtered.find((pack) => pack.id === selectedId) : undefined;
    if (fromFiltered) return fromFiltered;
    const fromStatic = selectedId ? getKnowledgePackById(selectedId) : undefined;
    if (fromStatic && filtered.some((pack) => pack.id === fromStatic.id)) return fromStatic;
  ...
  useEffect(() => {
    ...
    if (!selected) return;
    if (selected.id === urlSelectedId) return;
    const u = new URLSearchParams(searchParams.toString());
    u.set("id", selected.id);
    router.replace(`${pathname}?${u.toString()}`, { scroll: false });
  }, [isProjectModal, selected, urlSelectedId, router, pathname, searchParams]);
  ...
  const setPackId = useCallback(
    ...
    [isProjectModal, router, pathname, searchParams],
  );
  ...
  return (
    <div
      data-testid="knowledge-packs-browser"
      data-knowledge-packs-variant={variant}
      data-project-id={p.projectId?.trim() || undefined}
      style={{
        flex: 1,
        minHeight: 0,
        display: "flex",
    ...
    >
      ...
      >
        <label style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: 12, fontWeight: 800, color: t.textSecondary }}>
          ...
          <select
            ...
            data-testid="knowledge-packs-agent-filter"
          >
            {AGENTS.map((a) => (
        ...
        <label style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: 12, fontWeight: 800, color: t.textSecondary }}>
          ...
          <select
            ...
            data-testid="knowledge-packs-category-filter"
          >
            {CATEGORIES.map((c) => (
      ...
      {listLoading ? (
        ...
        <div style={{ maxWidth: isProjectModal ? "100%" : 760, width: "100%" }}>
