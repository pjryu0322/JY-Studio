...
export function buildDbIntegrationReviewResult(input: {
  ...
  const summary = newRequirementsMessage({
    ...
    content: [
      "DB 연동 필요성을 검토했습니다. 현재는 Mock 기반 구현을 유지하고, 전환 조건·엔티티 후보를 정리했습니다.",
      "",
      `- 저장 방식: mock (db_required=false 유지)`,
...
export function buildMockImplementationModeResult(input: {
  ...
  const summary = newRequirementsMessage({
    ...
    messageType: "STATEMENT",
    content: [
      "Mock 기반 구현 전략을 확정했습니다.",
      "",
      "- data_persistence_mode: mock",
      "- db_required: false",
      "- storage_strategy: Mock JSON / local state",
      "",
      "Code Agent WIP 작업 요청 흐름은 DB 슬롯 후보 상태와 무관하게 진행할 수 있습니다.",
      "",
      "다음 작업을 선택해 주세요.",
    ].join("\n"),
    createdAt: now,
