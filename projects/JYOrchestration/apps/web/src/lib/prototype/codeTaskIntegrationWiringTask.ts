...
export function buildIntegrationWiringCodeTask(input: {
  ...
  return {
    ...
    description:
      "foundation/data/common/feature/screen branch에서 생성된 컴포넌트, 상태, 샘플 데이터를 App Shell에 최종 연결하고 Preview 가능한 화면 흐름을 완성한다.",
    ...
    acceptanceCriteria: [
      ...
      "기존 WorkspaceShell/Panel 구조를 재작성하지 않고 연결 작업만 수행한다.",
      "생성된 screen component를 적절한 panel/slot에 배치한다.",
      "Loading/Error/Empty/Retry/Permission/Draft 공통 컴포넌트는 필요한 상태에 연결한다.",
      "샘플 데이터 provider를 화면 흐름에 연결한다.",
      "Preview 가능한 최종 화면 흐름을 완성한다.",
      "각 컴포넌트 내부 구현을 재작성하지 않는다.",
    ],
    verificationHints: [
      "통합 branch에서 Preview 화면 흐름 확인",
      "Shell/global 파일 중복 수정 없음",
      "개별 feature/screen/common 컴포넌트 내부 재작성 없음",
    ],
