...
const CLOSURE_VERSION = "stage_5_a_closure_v1" as const;
const CLOSURE_TITLE = "Stage 5-A Role Knowledge Binding Closure Package (Read-Only)";

const STAGE5_A_CLOSURE_BOUNDARY_REPORT = {
...
function parseClosureInput(input?: RoleKnowledgeBindingClosureInput) {
  const agentTypes =
  ...
  const availableKnowledgePackIds =
...
function buildSourceReadinessSnapshotWithHygiene(input: {
...
function buildClosureSummary(decision: RoleKnowledgeBindingClosureDecision): string {
  if (decision === "blocked") {
  ...
  return mapChecklistEntries([
    {
      ...
      detail: `blockedAgentCount=${input.snapshot.agentSummaries.filter((s) => s.decision === "blocked").length}`,
    },
    {
      item: "all source agent readiness ready",
      satisfied: input.snapshot.allReady,
      detail: `readyAgentCount=${input.snapshot.agentSummaries.filter((s) => s.decision === "knowledge_binding_ready").length}`,
    },
    {
    ...
    },
    {
      ...
      satisfied: input.stage5AClosureReviewConfirmed,
      detail: `stage5AClosureReviewConfirmed=${input.stage5AClosureReviewConfirmed}`,
    },
    {
      item: "not knowledge pack implementation confirmed",
      satisfied: input.stage5ANotKnowledgePackImplementationConfirmed,
      detail: `stage5ANotKnowledgePackImplementationConfirmed=${input.stage5ANotKnowledgePackImplementationConfirmed}`,
    },
    {
      item: "no RAG confirmed",
      satisfied: input.stage5ANoRagConfirmed,
      detail: `stage5ANoRagConfirmed=${input.stage5ANoRagConfirmed}`,
    },
    {
      item: "no prompt injection confirmed",
      satisfied: input.stage5ANoPromptInjectionConfirmed,
      detail: `stage5ANoPromptInjectionConfirmed=${input.stage5ANoPromptInjectionConfirmed}`,
    },
    {
      item: "no runtime/DB/UI confirmed",
      satisfied: input.stage5ANoRuntimeDbUiConfirmed,
      detail: `stage5ANoRuntimeDbUiConfirmed=${input.stage5ANoRuntimeDbUiConfirmed}`,
    },
    {
...
function buildBoundaryChecklist(): RoleKnowledgeBindingClosureChecklistItem[] {
  return mapChecklistEntries([
    {
      item: "stage5AClosureIsKnowledgePackImplementation=false",
      satisfied: true,
      detail: "stage5AClosureIsKnowledgePackImplementation=false",
    },
    {
      item: "stage5AClosureUsesRag=false",
      satisfied: true,
      detail: "stage5AClosureUsesRag=false",
    },
    {
      item: "stage5AClosureModifiesPromptInjection=false",
      satisfied: true,
      detail: "stage5AClosureModifiesPromptInjection=false",
    },
    {
      item: "stage5AClosureModifiesRuntime=false",
      satisfied: true,
      detail: "stage5AClosureModifiesRuntime=false",
    },
    {
      item: "stage5AClosureModifiesDb=false",
      satisfied: true,
      detail: "stage5AClosureModifiesDb=false",
    },
    {
      item: "stage5AClosureModifiesUi=false",
      satisfied: true,
      detail: "stage5AClosureModifiesUi=false",
    },
    {
    ...
    {
      item: "actualKnowledgePackCrudAllowedInThisStep=false",
      satisfied: true,
      detail: "actualKnowledgePackCrudAllowedInThisStep=false",
    },
    {
      item: "actualRagIndexingAllowedInThisStep=false",
      satisfied: true,
      detail: "actualRagIndexingAllowedInThisStep=false",
    },
    {
      item: "actualPromptInjectionAllowedInThisStep=false",
      satisfied: true,
      detail: "actualPromptInjectionAllowedInThisStep=false",
    },
  ]);
}
...
function appendClosureFindings(input: {
  ...
  readonly snapshot: SourceReadinessSnapshot;
  readonly parsed: ReturnType<typeof parseClosureInput>;
}): void {
  const { findings, decision, snapshot, parsed } = input;

  findings.push(finding("info", "stage5_a_closure_evaluator_created", "Stage 5-A closure evaluator created"));
  findings.push(finding("info", "stage5_a_closure_read_only", "Stage 5-A closure is read-only"));
  findings.push(finding("info", "stage5_a_source_readiness_aggregated", "Source readiness reports were aggregated"));
  findings.push(
    finding("info", "stage5_a_not_knowledge_pack_implementation", "Stage 5-A is not knowledge pack implementation"),
  );
  findings.push(finding("info", "stage5_a_no_rag_indexing", "Stage 5-A closure does not use RAG indexing"));
  findings.push(finding("info", "stage5_a_no_prompt_injection", "Stage 5-A closure does not modify prompt injection"));
  findings.push(
    finding("info", "stage5_a_no_runtime_db_ui_change", "Stage 5-A closure does not change runtime/DB/UI"),
  );
  findings.push(
    finding(
      "info",
      "stage5_b_metadata_registry_candidate_only",
      "Stage 5-B knowledge pack metadata registry is candidate only",
    ),
  );

  if (snapshot.hasBlocked) {
    findings.push(finding("blocking", "source_agent_readiness_blocked", "One or more source agent readiness reports are blocked"));
  ...
  if (decision === "defer") {
    if (snapshot.hasDefer) {
      findings.push(finding("warning", "source_agent_readiness_deferred", "One or more source agent readiness reports defer"));
    }
    if (snapshot.anyUnknown) {
      findings.push(
        finding("warning", "source_unknown_knowledge_pack_id_reported", "Unknown knowledge pack IDs were reported in source readiness"),
      );
    }
    if (snapshot.anyOptionalMissing) {
      findings.push(
        finding("warning", "source_optional_knowledge_pack_missing", "Optional knowledge packs are missing in source readiness"),
      );
    }
    if (!parsed.stage5AClosureReviewConfirmed) {
      findings.push(finding("warning", "closure_confirmation_missing", "Stage 5-A closure review confirmation is missing"));
    }
...
export function evaluateRoleKnowledgeBindingClosure(
  ...
  const parsed = parseClosureInput(input);
  const sourceDefaultKnowledgePackIds = sortedDefaultKnowledgePackIds();
  const snapshot = buildSourceReadinessSnapshotWithHygiene({
  ...
  appendClosureFindings({ findings, decision, snapshot, parsed });

  const noUnknownKnowledgePackIds = !snapshot.anyUnknown;
  const noBlankKnowledgePackIdsRemoved = !snapshot.anyBlankRemoved;
  const noDuplicateKnowledgePackIdsRemoved = !snapshot.anyDuplicateRemoved;
  ...
  return {
    ...
    noUnknownKnowledgePackIds,
    noBlankKnowledgePackIdsRemoved,
    noDuplicateKnowledgePackIdsRemoved,
    closureChecklist: buildClosureChecklist({
      snapshot,
      allRequiredBindingsSatisfied,
      stage5AClosureReviewConfirmed: parsed.stage5AClosureReviewConfirmed,
      stage5ANotKnowledgePackImplementationConfirmed: parsed.stage5ANotKnowledgePackImplementationConfirmed,
    ...
    boundaryChecklist: buildBoundaryChecklist(),
    findings,
