...
- (예시) 액터/권한/승인/공유/채널/디바이스/예외 흐름 같은 축으로 순서를 구성하되, 항상 1개만 묻는다.
- latestAiQuestion과 updatedFlow/readiness를 참고해 "다음 질문"을 결정한다.
- userMessage가 "인터뷰 시작"으로 시작하면, 프로젝트 설명과 아이디어 구체화 산출물을 바탕으로
  "첫 질문"을 1개만 만든다(사용자가 쉽게 답할 수 있게). quickReplies는 가능한 경우에만(최대 3개) 제안한다.
