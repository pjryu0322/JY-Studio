...
export function buildDynamicServicePlanningSlotDefinitions(input: {
  ...
  const k = {
    ...
    actorFunctionMatrix: `${p}.flow.actorFunctionMatrix`,
    actorPermissionMatrix: `${p}.flow.actorPermissionMatrix`,
    processActorMap: `${p}.flow.processActorMap`,
    processScreenMap: `${p}.flow.processScreenMap`,
    screenActorMatrix: `${p}.design.screenActorMatrix`,
    screenActionMatrix: `${p}.design.screenActionMatrix`,
    screenDataMap: `${p}.design.screenDataMap`,
    commonDetailFeatures: `${p}.design.commonDetailFeatures`,
    dataEntities: `${p}.design.dataEntities`,
    stateModel: `${p}.design.stateModel`,
    mockDataStrategy: `${p}.design.mockDataStrategy`,
    implementationSeedSummary: `${p}.design.implementationSeedSummary`,
  };
  ...
  const defs: SingleChatOrchestrationSlotDefinition[] = [
    ...
    {
      slotKey: k.actorFunctionMatrix,
      label: "액터별 기능 매핑",
      ownerAgent: "service-designer",
      stageGroup: SINGLE_CHAT_SERVICE_PLANNING_GROUP,
      hints: "각 액터가 수행할 기능을 목록으로 정리.",
      dependsOn: [k.actorTypes, k.coreFeatures],
    },
    {
      slotKey: k.actorPermissionMatrix,
      label: "액터별 권한/제약",
      ownerAgent: hasSecurity ? "security-reviewer" : "service-designer",
      stageGroup: SINGLE_CHAT_SERVICE_PLANNING_GROUP,
      hints: "동일 화면·기능에서 액터별 권한 차이.",
      dependsOn: [k.permissionRelations, k.actorFunctionMatrix],
    },
    {
      slotKey: k.processActorMap,
      label: "프로세스별 참여 액터",
      ownerAgent: "service-designer",
      stageGroup: SINGLE_CHAT_SERVICE_PLANNING_GROUP,
      hints: "프로세스 단계별 참여 액터.",
      dependsOn: [k.serviceFlow, k.actorTypes],
    },
    {
      slotKey: k.processScreenMap,
      label: "프로세스-화면 연결",
      ownerAgent: "service-designer",
      stageGroup: SINGLE_CHAT_SERVICE_PLANNING_GROUP,
      hints: "업무 단계 → 화면 매핑(예: 업로드 → 업로드 화면).",
      dependsOn: [k.serviceFlow, k.requiredScreens],
    },
    {
      slotKey: k.screenActorMatrix,
      label: "화면별 접근 액터",
      ownerAgent: "solution-architect",
      stageGroup: SINGLE_CHAT_SERVICE_PLANNING_GROUP,
      hints: "화면별 접근 가능 액터.",
      dependsOn: [k.requiredScreens, k.actorTypes],
    },
    {
      slotKey: k.screenActionMatrix,
      label: "화면별 기능/버튼",
      ownerAgent: "solution-architect",
      stageGroup: SINGLE_CHAT_SERVICE_PLANNING_GROUP,
      hints: "화면별 버튼·액션·입출력 요소.",
      dependsOn: [k.requiredScreens, k.coreFeatures],
    },
    {
      slotKey: k.screenDataMap,
      label: "화면별 데이터 표시/입력 항목",
      ownerAgent: "task-reviewer",
      stageGroup: SINGLE_CHAT_SERVICE_PLANNING_GROUP,
      hints: "화면별 표시·입력·수정 데이터.",
      dependsOn: [k.screenActionMatrix, k.dataFlow],
    },
    {
      slotKey: k.commonDetailFeatures,
      label: "공통 상세기능",
      ownerAgent: "task-reviewer",
      stageGroup: SINGLE_CHAT_SERVICE_PLANNING_GROUP,
      hints: "로딩·오류·빈 결과·재시도·권한 안내 등 공통 UX.",
      dependsOn: [k.coreFeatures],
    },
    {
      slotKey: k.dataEntities,
      label: "데이터 엔티티 후보",
      ownerAgent: "solution-architect",
      stageGroup: SINGLE_CHAT_SERVICE_PLANNING_GROUP,
      hints: "Mock/DB 전환 후보 엔티티.",
      dependsOn: [k.dataFlow, k.coreFeatures],
    },
    {
      slotKey: k.stateModel,
      label: "상태 모델",
      ownerAgent: "domain-expert",
      stageGroup: SINGLE_CHAT_SERVICE_PLANNING_GROUP,
      hints: "프로세스·화면·데이터 상태 전이.",
      dependsOn: [k.userStateChange, k.serviceFlow],
    },
    {
      ...
      dependsOn: [k.dataEntities, k.architecturePrototypeBoundary],
    },
    {
      slotKey: k.implementationSeedSummary,
      label: "구현 작업안 Seed",
      ownerAgent: "task-reviewer",
      stageGroup: SINGLE_CHAT_SERVICE_PLANNING_GROUP,
      hints: "프로세스·화면·액터·공통기능·데이터를 구현 초안용으로 요약.",
      dependsOn: [
        k.processScreenMap,
        k.screenActionMatrix,
        k.actorFunctionMatrix,
        k.commonDetailFeatures,
        k.dataEntities,
      ],
    },
  ];
