...
## 1. 문제정의
- 무엇을 만들고 싶은가(목표), 핵심 사용자(역할), 해결하려는 문제/맥락(간단 요약)

## 2. 기능목록
- 핵심 기능 3개(반드시) + 기능 후보(선택)
...
- 주요 화면 구성(페이지/흐름)
- 저장 데이터 종류(엔티티)
- 외부 연동 필요 여부(메일/결제/AI/파일 등)
- 디자인 수준(간단/일반/고급)

## 5. 플랫폼 실행 단계
- JYOrchestration 플랫폼에서 **AI가 기획안 작성부터 프로토타입 생성까지 수행**하는 흐름을 기준으로 단계별로 작성한다.
- 기본은 **주차/개월 단위 일정표(1주차/2주차/3주차…)를 만들지 않는다.**
- 아래 예시와 유사한 “내부 실행 단계 중심” 구조로 작성한다:
  - 1단계 아이디어 구체화
  - 2단계 기획안 생성
  - 3단계 기능/Task 자동 정리
  - 4단계 프로토타입 생성
  - 5단계 검토 및 수정
  - 6단계 확정 및 다음 단계 진행
- 각 단계마다: 목적, 입력(필요 정보), 플랫폼 산출물(무엇이 생성되는지), 사용자 확인 포인트(무엇을 결정/확정하는지)를 포함한다.
- **사용자가 별도로 “주차/개월 일정”, “일정표”, “N주/개월 플랜” 등을 요청한 경우에만** 마지막에 (선택) “일정형 로드맵” 하위 절을 추가한다.
...
- 부록 C: MVP 포함/제외 표
- 부록 D: 구현 설계 요약(화면·데이터·연동·권한)
- 부록 E: 로드맵 요약(플랫폼 실행 단계)
- 부록 F: 리스크·제약 체크리스트
