...
4. 우선순위`,
  ...
  full_plan: `아이디어 초안(단일 마크다운 문서 — 아이디어 구체화 단계 산출물):
반드시 아래 "##" 제목을 그대로 사용하고, 각 절 아래에 **고수준(개략)** 본문을 작성한다.
중요: 상세 액터 시나리오·권한 매트릭스·상세 화면 플로우·CRUD/DB/API 상세는 쓰지 않는다(다음 탭에서 다룸).

## 1. 서비스 개요
- 무엇을 만들고 싶은가(서비스 아이디어 한 문장)

## 2. 타겟 사용자
- 주 사용자(역할) 1~2개

## 3. 해결 문제
- 현재 가장 큰 불편/문제 1개(핵심만)

## 4. 기대 효과
- 어떻게 개선되길 원하는가(목표 상태/기대 변화)

## 5. 개략 액터
- 사용자 종류를 개략적으로만(예: 일반 사용자 / 관리자)

## 6. 개략 흐름
- 서비스 흐름을 한 줄 수준으로만(예: 업로드 → 분석 → 결과 확인)

## 7. 핵심 기능
- 반드시 필요한 핵심 기능 3개 내외(동사형)

## 8. 제약사항
- 예산/기간/정책/보안 등 큰 제약

## 9. 다음 탭으로 넘길 질문(핵심 3~6개)
- 액터/서비스 흐름 정의 탭에서 구체화할 항목
- 기능 정리 탭에서 구체화할 항목
- 작업 정리 탭에서 구체화할 항목`,
};
