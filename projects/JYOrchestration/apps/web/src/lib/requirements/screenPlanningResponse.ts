/**
 * Project SingleChat — screen planning MVP responses (not service-flow analyze).
 */
...
export function validateScreenPlanningAssistantMessage(assistantMessage: string): {
  ...
  if (t.length < 80) issues.push("screen_planning_message_too_short");
...
export function buildScreenPlanningAssistantMessage(input: {
  ...
  if (steps.length >= 3) {
    ...
    defaults.forEach((row, i) => {
      ...
      lines.push(`- ${row.bullet}`);
      lines.push("");
    });
  }

  lines.push("다음: 이 화면 구성을 기능 범위로 정리할 수 있습니다.");
  return lines.join("\n").trim();
}
