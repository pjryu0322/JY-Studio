...
export async function POST(
  ...
  try {
    ...
    void request;
    const row = await retryAiMemberActionRequest(actionId, userId);
    ...
    console.error("POST /api/ai-member-actions/[actionId]/retry error:", error);
    return NextResponse.json(
      { success: false, message: "재시도 큐잉 중 오류가 발생했습니다." },
      { status: 500 }
    );
  }
}
