...
import { runActorFlowOrganizeOpenAI } from "@/lib/actor-flow/actorFlowOrganizeOpenAI";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const dialogueExcerpt = String(body.dialogueExcerpt ?? "").trim();
    ...
    try {
      await requireProjectPermission(projectId, userId, "canViewProject", "POST /api/actor-flow/organize");
    } catch (error) {
    ...
    const result = await runActorFlowOrganizeOpenAI({
      projectName,
      projectDescription,
      dialogueExcerpt,
      ideationAssets,
    ...
    const denied = rbacErrorResponse(error);
    if (denied) return denied;
    console.error("POST /api/actor-flow/organize error:", error);
    return NextResponse.json({ success: false, message: "정리 요청 처리 중 오류가 발생했습니다." }, { status: 500 });
  }
}
