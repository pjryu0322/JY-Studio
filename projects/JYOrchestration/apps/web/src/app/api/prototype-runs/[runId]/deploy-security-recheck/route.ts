...
export async function POST(
  ...
  try {
    await requireProjectPermission(projectId, userId, "canViewProject", "POST /api/prototype-runs/[runId]/deploy-security-recheck");
  } catch (error) {
  ...
  const started = startPrototypeDeploySecurityCheck(projectId, id, { isRecheck: true });
  if (!started) {
    return NextResponse.json(
      { success: false, message: "재점검을 시작할 수 있는 상태가 아닙니다. 조치 완료 후 또는 조치 필요 상태에서 시도하세요." },
      { status: 409 },
    );
  }
