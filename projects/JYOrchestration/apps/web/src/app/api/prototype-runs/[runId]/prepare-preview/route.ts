...
import { preparePrototypeRunPreview } from "@/lib/prototype/prototypeRunPipeline";
...
export async function POST(
  ...
  try {
    await requireProjectPermission(projectId, userId, "canViewProject", "POST /api/prototype-runs/[runId]/prepare-preview");
  } catch (error) {
  ...
  const run = (await preparePrototypeRunPreview(projectId, id)) ?? existing;
