...
export async function POST(request: NextRequest) {
  ...
  appendReviewMessage(projectId, runId, "user", "[정리요청] 지금까지의 검토 내용을 AI기획자 관점에서 짧게 묶어 주세요.");
  const messages = getReviewThread(projectId, runId);
  const run = getRun(projectId, runId);

  const system = `당신은 JYOrchestration의 AI기획자다. 프로토타입 검토 대화를 사용자가 보기 좋게 요약한다.
