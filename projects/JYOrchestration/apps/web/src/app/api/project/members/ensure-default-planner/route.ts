...
/**
 * 아이디어 구체화 등에서 spec 단계 기본 AI 기획자(planner) 멤버가 없을 때 보강합니다.
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, message: "기본 AI 기획자 멤버가 준비되었습니다." });
  } catch (error) {
    const denied = rbacErrorResponse(error);
    if (denied) {
      return denied;
    ...
    return NextResponse.json(
      { success: false, message: "기본 AI 기획자 멤버를 준비하는 중 오류가 발생했습니다." },
      { status: 500 }
    );
  }
}
