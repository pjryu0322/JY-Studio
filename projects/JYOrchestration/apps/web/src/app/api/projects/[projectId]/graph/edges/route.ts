...
import { listProjectGraphEdges } from "@/lib/project-graph/projectGraphQuery";
...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    try {
      await requireProjectPermissionById(pid, userId, "canViewProject", "GET /api/projects/[projectId]/graph/edges");
    } catch (error) {
    ...
    const edges = await listProjectGraphEdges(pid, {
      edgeType,
    ...
    return NextResponse.json({
      success: true,
      message: "프로젝트 그래프 엣지를 조회했습니다.",
      data: { edges },
    });
  } catch (error) {
    const denied = rbacErrorResponse(error);
    if (denied) return denied;
    console.error("GET /api/projects/[projectId]/graph/edges error:", error);
