...
/**
 * 프로젝트 소유자만 저장된 Cursor API 키 전체를 일시 확인할 수 있습니다.
...
export async function POST(
  ...
  try {
    ...
    try {
      await requireProjectOwnedByUser(pid, userId, "POST .../cursor-token/reveal");
    } catch (error) {
    ...
    const row = await withExecutionSetupSchemaHealRetry(() =>
      prisma.executionSetup.findUnique({
        where: { projectId: pid },
        select: { cursorApiToken: true },
      })
    );

    const tok = row?.cursorApiToken?.trim();
    if (!tok) {
      return NextResponse.json(
        { success: false, message: "저장된 Cursor API 키가 없습니다." },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json({ success: false, message: "키를 불러오지 못했습니다." }, { status: 500 });
  }
}
