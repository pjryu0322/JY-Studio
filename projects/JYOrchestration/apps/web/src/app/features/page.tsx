...
function FeaturesPageInner() {
  ...
  const serviceFlowHref = useMemo(
    () => (projectId ? `/requirements?projectId=${encodeURIComponent(projectId)}&stage=service-flow` : "/requirements?stage=service-flow"),
    [projectId]
  );
        ...
        {projectId ? (
          <WorkflowCard padding={16}>
            ...
            <div style={{ marginTop: 12 }}>
              <Link href={serviceFlowHref} style={btnSecondary}>
                액터 및 서비스 흐름 정의로 이동
              </Link>
            </div>
          </WorkflowCard>
