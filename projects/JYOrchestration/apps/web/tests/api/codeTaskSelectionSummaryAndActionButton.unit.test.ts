...
import { summarizeSelectableCodeTasks } from "@/lib/prototype/implementationCodeTaskSelectionSummary";
...
describe("P3-08C selection summary and actions", () => {
  ...
  it("defaults selection mode to execution", () => {
    expect(resolveCodeTaskSelectionMode({})).toBe("execution");
  });

  it("reports runnableCount=1 when only sample data is execution-eligible", () => {
    const summary = summarizeSelectableCodeTasks({
      codeTasks,
      units,
      runs,
  ...
  it("primary action is execute_selected when sample data is selected", () => {
    ...
    expect(action.primaryAction).toBe("execute_selected");
  ...
  it("primary action is integration when only completed tasks selected", () => {
    const action = resolveImplementationBoardPrimaryAction({
      selectedCodeTaskIds: completedIds,
      codeTasks,
      units,
      runs,
      progressByCodeTaskId: progress,
