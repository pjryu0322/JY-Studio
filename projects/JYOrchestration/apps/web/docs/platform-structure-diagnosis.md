...
- **단계**: Contract(1) → Runtime Metadata(2) → Policy Helper(3) → Diagnostic / Warning(4) → **Runtime Diagnostic / Selection Preparation(5, 현재)** → Policy-guided Context Assembly(6, 다음 단계 준비; **미도입**) → Enforcement(7, **미도입**).
  - 진단 API의 `overlayArchitecturePhase.current = "runtime-diagnostic-selection-preparation-layer"`, `overlayMaturity.runtimeDiagnosticSelectionPreparationLayer = true`.
...
- **Runtime Diagnostic / Selection Preparation (현재 단계)** — 모두 read-only optional metadata. **prompt 본문·OpenAI payload·라우팅 비변경**, **자동 orchestration / 자동 retrieval / 자동 provider 선택 없음**.
  ...
  - **진단 API 확장**: `overlay-runtime` 응답에 **`overlaySelectionSummary`**·**`overlayConflictSummary`**·**`overlayContextBudgetSummary`** 추가(`?projectId=` 동반 시 마지막 promptTrace 기반). `overlayArchitecturePhase.current` = `"runtime-diagnostic-selection-preparation-layer"`.
- **단위 테스트**: `tests/overlay/*.unit.test.ts`(`overlayContextSelection`, `overlayContextBudget`, `overlayConflictDetection`, `overlayOrchestrationDecisionTrace`, **`overlayPromptTraceReplay`**, **`overlayPromptTraceAugmentConflict`** 포함), `tests/api/overlayPromptTracePersistence.unit.test.ts`, `tests/api/executionReviewOverlayWarningSummary.unit.test.ts`, **`tests/api/overlayRuntimeDiagnosticReplay.unit.test.ts`**.
