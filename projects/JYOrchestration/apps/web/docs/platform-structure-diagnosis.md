...
- **Harness Phase H6 Preparation — Review / Security Harness (17단계, 현재; planning metadata only)** — "AI검수자(AI Reviewer)와 AI보안관(AI Security Auditor)이 어떤 기준으로 결과물을 검토해야 하는가"를 review/security checklist planning metadata로 설명한다. **실제 보안 스캔·코드 리뷰 실행·이슈 등록·머지 차단·PR 게이트·remediation 자동 실행 어디에도 영향 없음.** 정책 표는 강제 규칙이 아니라 **검토 기준 추천**일 뿐이다.
