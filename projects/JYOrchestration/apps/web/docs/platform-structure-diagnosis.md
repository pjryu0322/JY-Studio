...
- **Harness Phase H1 — Controlled Prompt Assembly Preview (10단계, 현재; dry-run only)** — Harness가 표준 방식으로 prompt를 조립한다면 어떤 prompt가 만들어질지 **미리보기(preview)** 만 생성한다. **실제 prompt payload, OpenAI 호출, retrieval, provider, Cursor execution 어디에도 영향 없음.**
