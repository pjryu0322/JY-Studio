...
- **단계**: Contract(1) → Runtime Metadata(2) → Policy Helper(3) → Diagnostic / Warning(4) → Runtime Diagnostic / Selection Preparation(5) → Policy-guided Context Assembly Preparation(6) → **Policy-guided Assembly Plan Stabilization(7, 현재)** → Controlled Prompt Assembly Preparation(8, 다음 단계 준비; **미도입**) → Enforcement(9, **미도입**).
...
- **Policy-guided Assembly Plan Stabilization (7단계, 현재)** — selection → prioritization → assembly plan → pruning candidate → drift 흐름을 안정적으로 연결하고 replay·diagnostic 가능하게 만든다.
