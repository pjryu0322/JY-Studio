...
- **단계**: Contract(1) → Runtime Metadata(2) → Policy Helper(3) → Diagnostic / Warning(4) → Runtime Diagnostic / Selection Preparation(5) → **Policy-guided Context Assembly Preparation(6, 현재)** → Policy-guided Prompt Assembly(7, 다음 단계 준비; **미도입**) → Enforcement(8, **미도입**).
  - 진단 API의 `overlayArchitecturePhase.current = "policy-guided-context-assembly-preparation-layer"`, `overlayArchitecturePhase.autoPromptAssemblyEnabled = false`, `overlayMaturity.policyGuidedContextAssemblyPreparationLayer = true`.
...
- **Policy-guided Context Assembly Preparation (6단계, 현재)** — 모두 read-only **planning metadata**. 실제 prompt assembly·자동 pruning·자율 orchestration **여전히 금지**.
  - `overlayContextAssemblyPlan.ts` — `buildOverlayContextAssemblyPlan`(selection refs → `{type, source, priority, includeReason, estimatedCost, pruningCandidate}` plan items, role은 plan에서 제외). overflow 상태에 따라 timeline/workspace/knowledge를 pruningCandidate로 표시. `parseOverlayAssemblyPlanFromUnknown`·`summarizeOverlayAssemblyPlan`(highPriority 임계 `OVERLAY_ASSEMBLY_PLAN_HIGH_PRIORITY_THRESHOLD`).
  - `overlayContextPrioritization.ts` — `prioritizeOverlayContexts({contexts, budgetPolicy})` — `compact|balanced|default|extended` 별 type weight로 selection refs sorting only(payload/실제 assembly 비변경).
  - `overlayContextPruning.ts` — `suggestOverlayPruningCandidates({assemblyPlan, overflowRisk})`(low는 빈 배열, medium/high만 후보 제안). `parseOverlayPruningCandidatesFromUnknown`·`summarizeOverlayPruningCandidates`. **suggestion only**.
  - `overlayPolicyDriftWarning.ts` — `detectOverlayPolicyDrift({assemblyPlan, budgetMetadata})` — 휴리스틱: `OVERLAY_DRIFT_COMPACT_TIMELINE_OVERLOAD`, `OVERLAY_DRIFT_OVERFLOW_HIGH_WITHOUT_PRUNING`, `OVERLAY_DRIFT_NO_MEMORY_SCOPE`, `OVERLAY_DRIFT_NO_KNOWLEDGE_SCOPE`. **warning only**(`enforcement: "not_applied"`).
  - **augment 연결**: `buildOrchestrationOverlayPromptTraceAugments`가 selection refs/budget을 기반으로 `overlayContextAssemblyPlan`·`overlayPruningCandidates`를 생성해 timeline entry에 optional 부착. **prompt 본문 변경 없음**.
  - **PromptTrace 직렬화**: `requirementsStateJson` 타임라인 entry가 optional **`overlayContextAssemblyPlan`**·**`overlayPruningCandidates`** 보존. `coerceOverlayPromptTracePreparationMetadata`가 extract/coerce 양쪽에서 dispatch. 행당 plan 최대 `OVERLAY_ASSEMBLY_PLAN_ITEMS_MAX`, pruning 최대 `OVERLAY_PRUNING_CANDIDATES_MAX`(invalid 행은 drop).
  - **진단 API 확장**: `overlay-runtime` 응답에 **`overlayAssemblyPlanSummary`**·**`overlayPruningSummary`**·**`overlayPolicyDriftWarnings`** 추가. `overlayArchitecturePhase.current` = `"policy-guided-context-assembly-preparation-layer"`, `autoPromptAssemblyEnabled: false`, `overlayMaturity.policyGuidedContextAssemblyPreparationLayer = true`.
- **단위 테스트**: `tests/overlay/*.unit.test.ts`(기존 5단계 셋트 + **`overlayContextAssemblyPlan`**, **`overlayContextPrioritization`**, **`overlayContextPruning`**, **`overlayPolicyDriftWarning`**, **`overlayAssemblyReplay`** 포함), `tests/api/overlayPromptTracePersistence.unit.test.ts`, `tests/api/executionReviewOverlayWarningSummary.unit.test.ts`, `tests/api/overlayRuntimeDiagnosticReplay.unit.test.ts`.
