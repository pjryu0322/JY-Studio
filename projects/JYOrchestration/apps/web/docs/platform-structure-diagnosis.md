...
- **단계**: Contract(1) → Runtime Metadata(2) → Policy Helper(3) → Diagnostic / Warning(4) → Runtime Diagnostic / Selection Preparation(5) → Policy-guided Context Assembly Preparation(6) → Policy-guided Assembly Plan Stabilization(7) → Overlay Observability UI Phase 1(8) → Overlay Observability UI Phase 1.5(9) → Harness Phase H1 — Controlled Prompt Assembly Preview(10) → Harness Phase H2 — Apply-readiness Preparation(11) → Harness Phase H3 — Role-aware Knowledge Activation(12) → Harness Phase H4 Preparation — Memory Runtime Harness(13) → Harness Phase H4.5 — Memory Runtime Harness Stabilization(14) → Harness Phase H5 Preparation — Execution Routing Harness(15) → Harness Phase H5.5 — Execution Routing Safety & Explainability Stabilization(16) → Harness Phase H6 Preparation — Review / Security Harness(17; planning metadata only) → **Harness Phase H6.5 — Review / Security Safety & Issue Planning(18; planning metadata only)** → **Harness Phase H7 — Message-level Explainability UI(19, 현재; SingleChat AI 응답 read-only 패널)** → **Harness Phase H7.5 — Explainability UX Stabilization(20, 현재; read-only)** → **Harness Phase H7.6 — SingleChat Explainability Integration Verification & Wiring(21, 현재; read-only)** → Enforcement(22, **미도입**). Harness 순서는 H1 → H2 → H3 → H4 → H4.5 → H5 → H5.5 → H6 → H6.5 → H7 → H7.5 → H7.6로 정렬되어 있다.
  - **진단 API의 Harness phase(현재 구현)**: `overlayArchitecturePhase.current`는 **`harness-review-security-issue-planning-layer`**(H6.5)이며, `harnessReviewSecurityIssuePlanningEnabled: true`, `overlayMaturity.harnessReviewSecurityIssuePlanningLayer: true`가 응답에 포함된다(H6 checklist 단계 플래그 `harnessReviewSecurityPreparation*` 등은 호환을 위해 병행 true일 수 있음). **`harnessControlledRuntimeTrialPreparationEnabled: true`**(H10), **`harnessControlledRuntimeGovernanceEnabled: true`**(H10.5), **`harnessRuntimeEnforcementCandidateLayerEnabled: true`**(H11), **`harnessControlledEnforcementGovernanceEnabled: true`**(H11.5), **`harnessRuntimeStabilityPlanningEnabled: true`**(H12), **`harnessRuntimePlanningPriorityEscalationEnabled: true`**(H12.5), **`harnessRuntimePlanningLifecycleGovernanceEnabled: true`**(H13.5), **`harnessRuntimePlanningCoherenceSynchronizationEnabled: true`**(H14), **`harnessRuntimePlanningConsolidationNormalizationEnabled: true`**(H14.5), **`harnessRuntimePlanningDependencyImpactGraphEnabled: true`**(H15), **`harnessRuntimePlanningCriticalityPriorityPropagationEnabled: true`**(H15.5), **`harnessRuntimePlanningTraceabilityReasoningChainEnabled: true`**(H16), **`harnessRuntimePlanningReasoningConsolidationEnabled: true`**(H16.5), **`harnessRuntimePlanningSemanticCompressionEnabled: true`**(H17), **`harnessRuntimePlanningSemanticQualityGateEnabled: true`**(H17.5), **`harnessRuntimePlanningSemanticExplainabilityGraphEnabled: true`**(H18), **`harnessRuntimePlanningSemanticNarrativeConsolidationEnabled: true`**(H18.5), **`harnessRuntimePlanningSemanticVocabularyStabilizationEnabled: true`**(H19), **`harnessRuntimePlanningDecisionIntelligenceEnabled: true`**(H19.5), **`harnessRuntimePlanningForecastingEnabled: true`**(H20), **`harnessRuntimePlanningResourceIntelligenceEnabled: true`**(H20.5), **`harnessRuntimePlanningResourceGovernanceEnabled: true`**(H21), **`harnessRuntimePlanningResourceAllocationEnabled: true`**(H21.5), **`harnessRuntimePlanningResourceTrialEnabled: true`**(H22), **`harnessRuntimePlanningControlBoundaryEnabled: true`**(H22.5), **`harnessRuntimePlanningExecutionCandidateEnabled: true`**(H23), **`harnessRuntimePlanningOperatorApprovalReadinessEnabled: true`**(H23.5), **`harnessRuntimePlanningControlledOrchestrationRuntimePilotEnabled: true`**(H24), **`harnessRuntimePlanningPilotContractAdapterBoundaryEnabled: true`**(H24.5), **`harnessRuntimePlanningNoopRuntimeAdapterEnabled: true`**(H25), **`harnessRuntimePlanningNoopRuntimeAdapterStabilizationEnabled: true`**(H25.5), **`harnessRuntimePlanningRuntimeAdapterSandboxEnabled: true`**(H26), **`harnessRuntimePlanningRuntimeAdapterSandboxStabilizationEnabled: true`**(H26.5), **`harnessRuntimePlanningRuntimePilotActivationCandidateEnabled: true`**(H27), **`harnessRuntimePlanningRuntimePilotActivationStabilizationEnabled: true`**(H27.5), **`harnessRuntimePlanningIsolatedRuntimePilotSkeletonEnabled: true`**(H28), **`harnessRuntimePlanningIsolatedRuntimePilotSkeletonStabilizationEnabled: true`**(H28.5), **`harnessRuntimePlanningIsolatedDryRunRunnerInvocationCandidateEnabled: true`**(H29), **`harnessRuntimePlanningIsolatedDryRunRunnerInvocationStabilizationEnabled: true`**(H29.5), **`harnessRuntimePlanningIsolatedDryRunRunnerNoopHarnessEnabled: true`**(H30), **`harnessRuntimePlanningIsolatedDryRunRunnerNoopHarnessStabilizationEnabled: true`**(H30.5), **`harnessRuntimePlanningIsolatedDryRunNoopExecutionShellCandidateEnabled: true`**(H31), **`harnessRuntimePlanningIsolatedDryRunNoopExecutionShellStabilizationEnabled: true`**(H31.5), **`harnessRuntimePlanningIsolatedDryRunNoopShellHardeningEnabled: true`**(H32), **`overlayMaturity.harnessControlledEnforcementGovernanceLayer: true`**, **`overlayMaturity.harnessRuntimeStabilityPlanningLayer: true`**, **`overlayMaturity.harnessRuntimePlanningPriorityEscalationLayer: true`**, **`overlayMaturity.harnessRuntimePlanningLifecycleGovernanceLayer: true`**, **`overlayMaturity.harnessRuntimePlanningCoherenceSynchronizationLayer: true`**, **`overlayMaturity.harnessRuntimePlanningConsolidationNormalizationLayer: true`**, **`overlayMaturity.harnessRuntimePlanningDependencyImpactGraphLayer: true`**, **`overlayMaturity.harnessRuntimePlanningCriticalityPriorityPropagationLayer: true`**, **`overlayMaturity.harnessRuntimePlanningTraceabilityReasoningChainLayer: true`**, **`overlayMaturity.harnessRuntimePlanningReasoningConsolidationLayer: true`**, **`overlayMaturity.harnessRuntimePlanningSemanticCompressionLayer: true`**, **`overlayMaturity.harnessRuntimePlanningSemanticQualityGateLayer: true`**, **`overlayMaturity.harnessRuntimePlanningSemanticExplainabilityGraphLayer: true`**, **`overlayMaturity.harnessRuntimePlanningSemanticNarrativeConsolidationLayer: true`**, **`overlayMaturity.harnessRuntimePlanningSemanticVocabularyStabilizationLayer: true`**, **`overlayMaturity.harnessRuntimePlanningDecisionIntelligenceLayer: true`**, **`overlayMaturity.harnessRuntimePlanningForecastingLayer: true`**, **`overlayMaturity.harnessRuntimePlanningResourceIntelligenceLayer: true`**, **`overlayMaturity.harnessRuntimePlanningResourceGovernanceLayer: true`**, **`overlayMaturity.harnessRuntimePlanningResourceAllocationLayer: true`**, **`overlayMaturity.harnessRuntimePlanningResourceTrialLayer: true`**, **`overlayMaturity.harnessRuntimePlanningControlBoundaryLayer: true`**, **`overlayMaturity.harnessRuntimePlanningExecutionCandidateLayer: true`**, **`overlayMaturity.harnessRuntimePlanningOperatorApprovalReadinessLayer: true`**, **`overlayMaturity.harnessRuntimePlanningControlledOrchestrationRuntimePilotLayer: true`**, **`overlayMaturity.harnessRuntimePlanningPilotContractAdapterBoundaryLayer: true`**, **`overlayMaturity.harnessRuntimePlanningNoopRuntimeAdapterLayer: true`**, **`overlayMaturity.harnessRuntimePlanningNoopRuntimeAdapterStabilizationLayer: true`**(H25.5), **`overlayMaturity.harnessRuntimePlanningRuntimeAdapterSandboxLayer: true`**(H26), **`overlayMaturity.harnessRuntimePlanningRuntimeAdapterSandboxStabilizationLayer: true`**, **`harnessRuntimePlanningRuntimePilotActivationCandidateLayer: true`**(H27), **`harnessRuntimePlanningRuntimePilotActivationStabilizationLayer: true`**(H27.5), **`harnessRuntimePlanningIsolatedRuntimePilotSkeletonLayer: true`**(H28), **`harnessRuntimePlanningIsolatedRuntimePilotSkeletonStabilizationLayer: true`**(H28.5), **`harnessRuntimePlanningIsolatedDryRunRunnerInvocationCandidateLayer: true`**, **`harnessRuntimePlanningIsolatedDryRunRunnerInvocationStabilizationLayer: true`**(H29.5), **`harnessRuntimePlanningIsolatedDryRunRunnerNoopHarnessLayer: true`**(H30), **`harnessRuntimePlanningIsolatedDryRunRunnerNoopHarnessStabilizationLayer: true`**(H30.5), **`overlayMaturity.harnessRuntimePlanningIsolatedDryRunNoopExecutionShellCandidateLayer: true`**(H31), **`overlayMaturity.harnessRuntimePlanningIsolatedDryRunNoopExecutionShellStabilizationLayer: true`**(H31.5)도 동일 응답에 포함된다. 진단 필드 **`runtimeNoopExecutionShellSummary`**·**`runtimeNoopExecutionShellScope`**·**`runtimeNoopExecutionShellPolicy`**·**`runtimeNoopExecutionShellBlockerReport`**·**`runtimeNoopExecutionShellReadinessChecklist`**·**`runtimeNoopExecutionShellFinalSafetyGate`**·**`runtimeNoopExecutionShellBoundaryViolationReport`**·**`runtimeNoopExecutionShellReadinessVerificationReport`** additive. 진단 필드 **`runtimeAdapterSandboxSummary`**·**`runtimeAdapterSandboxInputEnvelope`**·**`runtimeAdapterSandboxOutputEnvelope`**·**`runtimeAdapterSandboxPolicy`**·**`runtimeAdapterSandboxResultMetadata`**·**`runtimeAdapterSandboxBlockerReport`**·**`runtimeAdapterSandboxEnvelopeVerificationReport`**·**`runtimeAdapterSandboxBoundaryViolationReport`**·**`runtimeAdapterSandboxPreflightSummary`**·**`runtimePilotActivationSummary`**·**`runtimePilotActivationScope`**·**`runtimePilotActivationPolicy`**·**`runtimePilotActivationBlockerReport`**·**`runtimePilotActivationReadinessChecklist`**·**`runtimePilotActivationFinalSafetyGate`**·**`runtimePilotActivationBoundaryViolationReport`**·**`runtimePilotActivationReadinessVerificationReport`**·**`runtimePilotSkeletonSummary`**·**`runtimeDryRunRunnerContract`**·**`runtimePilotRunnerInputEnvelope`**·**`runtimePilotRunnerOutputEnvelope`**·**`runtimePilotRunnerSafetyGuard`**·**`runtimePilotSkeletonBlockerReport`**·**`runtimePilotRunnerContractVerificationReport`**·**`runtimePilotRunnerBoundaryViolationReport`**·**`runtimePilotRunnerNoExecutionResultMetadata`**·**`runtimePilotSkeletonPreflightSummary`**·**`runtimeRunnerInvocationSummary`**·**`runtimeRunnerInvocationScope`**·**`runtimeRunnerInvocationPolicy`**·**`runtimeRunnerInvocationBlockerReport`**·**`runtimeRunnerInvocationReadinessChecklist`** additive. assembly plan 안정화 등 **이전 Overlay 단계**의 phase 문자열과는 별개이다.
...
- **Harness Phase H39 — Final Release Governance Gate Candidate Layer (read-only; planning metadata only)** — H38.5 governance release-readiness final safety gate를 바탕으로 **final release governance gate candidate·scope·policy·blocker report·readiness checklist** metadata만 산출한다. `apps/web/src/lib/harness/runtimeFinalReleaseGovernanceGate/*`: **`detectRuntimeFinalReleaseGovernanceGateBlockers`**, **`evaluateRuntimeFinalReleaseGovernanceGateCandidate`**, **`resolveRuntimeFinalReleaseGovernanceGateMode`**, **`buildRuntimeFinalReleaseGovernanceGateScope`**, **`buildRuntimeFinalReleaseGovernanceGatePolicy`**, **`buildRuntimeFinalReleaseGovernanceGateReadinessChecklist`**, **`buildRuntimeFinalReleaseGovernanceGatePlanningReports`**, **`serializeRuntimeFinalReleaseGovernanceGateDiagnosticBundleFromSemanticReports`**. Overlay **`OverlayRuntimeFinalReleaseGovernanceGateSection`**·**`overlayRuntimeFinalReleaseGovernanceGateSectionVm`**. 진단 **`runtimeFinalReleaseGovernanceGateSummary`**·**`runtimeFinalReleaseGovernanceGateScope`**·**`runtimeFinalReleaseGovernanceGatePolicy`**·**`runtimeFinalReleaseGovernanceGateBlockerReport`**·**`runtimeFinalReleaseGovernanceGateReadinessChecklist`** additive. Policy **`actualExecutionForbidden`**·**`actualExecutionBlockingForbidden`**·**`actualMergeBlockingForbidden`** true 유지. **여전히 금지**: actual execution·execution routing·release enforcement·approval enforcement·execution blocking·merge blocking·prompt 변경·DB 변경.

- **Harness Phase H39.5 — Final Release Governance Gate Stabilization & Ultimate No-Enforcement Safety Gate Layer (read-only)** — H39 candidate metadata를 **안정화**하고 ultimate no-enforcement **final safety gate**·**violation detection**·**readiness verification**·**alignment report**·**H40 entry readiness**(`h40EntryReadiness` = `finalGateStatus`)를 산출한다. **`detectRuntimeFinalReleaseGovernanceGateViolations`**, **`verifyRuntimeFinalReleaseGovernanceGateReadiness`**, **`buildRuntimeFinalReleaseGovernanceGateAlignmentReport`**, **`buildRuntimeFinalReleaseGovernanceGateFinalSafetyGate`**. 진단 **`runtimeFinalReleaseGovernanceGateViolationReport`**·**`runtimeFinalReleaseGovernanceGateVerificationReport`**·**`runtimeFinalReleaseGovernanceGateAlignmentReport`**·**`runtimeFinalReleaseGovernanceGateFinalSafetyGate`** additive. **actual execution forbidden**·**actual release enforcement forbidden**·**actual approval enforcement forbidden**·**actual execution blocking forbidden**·**actual merge blocking forbidden** 유지. **여전히 금지**: actual execution·execution routing·release enforcement·approval enforcement·execution blocking·merge blocking·routing·queue control·rollback·prompt 변경.
...
- **Harness Phase H42 — Limited Controlled Runtime Pilot Boundary Candidate Layer (read-only)** — H41.5 final safety gate를 바탕으로 limited pilot boundary summary·scope·policy·input/output contract·blocker·checklist metadata만 산출. 진단 7개 필드 additive. **여전히 pilot activation·execution·runner·adapter·sandbox 없음**.
- **Harness Phase H42.5 — Limited Pilot Boundary Stabilization & Final Pilot Boundary Safety Gate Layer (read-only)** — H42 candidate metadata를 안정화하고 violation·verification·alignment·**`h43EntryReadiness`** final safety gate 산출. 진단 4개 필드 additive. **여전히 pilot activation·execution·runner·adapter·sandbox 없음**.
- **Harness Phase H43 — Limited Runtime Pilot Readiness Review & Pilot Contract Hardening Boundary Layer (read-only)** — H42.5 final safety gate를 바탕으로 readiness review·contract hardening boundary·input/output envelope·no-execution/forbidden proof·checklist metadata만 산출. 진단 8개 필드 additive. **여전히 pilot activation·execution 없음**.
- **Harness Phase H43.5 — Limited Pilot Readiness Review Stabilization & Final Pilot Contract Safety Gate Layer (read-only)** — H43 readiness review metadata를 안정화하고 violation·verification·alignment·**`h44EntryReadiness`** final safety gate 산출. 진단 4개 필드 additive. **여전히 pilot activation·execution 없음**.
...
- **Pilot Validation Phase 3 — Validation Request Draft & Operator Approval UI Contract (read-only, 1차 완료선)** — request draft·approval snapshot·audit/rollback candidates. `/pilot-validation?projectId=` 사용자 검증 준비 화면. actual enforcement/rollback/invocation 없음. 완료 검증: `pilot-validation-phase3-completion.md`, `runtimePilotValidationPhase3CompletionVerification.unit.test.ts`.
- **Pilot Validation Phase 4 — Safe Echo Invocation Simulator Contract (read-only, 선택 확장)** — simulator summary/input/output/boundary metadata. diagnostic API additive 4 fields. actual adapter/sandbox/runner invocation 없음. Phase 3 완료선 이후 optional extension.
