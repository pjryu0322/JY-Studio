...
| **Context Assembly Plan(read-only)** | `apps/web/src/lib/overlay/overlayContextAssemblyPlan.ts` — `buildOverlayContextAssemblyPlan`·`parseOverlayAssemblyPlanFromUnknown`·`summarizeOverlayAssemblyPlan`·`OVERLAY_ASSEMBLY_PLAN_ITEMS_MAX` |
| **Context Prioritization(metadata sort only)** | `apps/web/src/lib/overlay/overlayContextPrioritization.ts` — `prioritizeOverlayContexts` (compact/balanced/default/extended) |
| **Pruning Candidate Suggestion(read-only)** | `apps/web/src/lib/overlay/overlayContextPruning.ts` — `suggestOverlayPruningCandidates`·`parseOverlayPruningCandidatesFromUnknown`·`summarizeOverlayPruningCandidates`·`OVERLAY_PRUNING_CANDIDATES_MAX` |
...
6. **Policy-guided Context Assembly Preparation Layer** (현재) — "무엇이 선택되었는가"에서 한 단계 더 들어가 **"무엇을 실제 prompt assembly에 우선 사용할 것인가"** 를 read-only로 *계획*한다. 신규 metadata: `overlayContextAssemblyPlan`(선택 ref→type/priority/includeReason/estimatedCost/pruningCandidate), `overlayPruningCandidates`(overflow 시 줄일 후보 suggestion). 신규 helper: `buildOverlayContextAssemblyPlan`·`prioritizeOverlayContexts`(compact/balanced/default/extended)·`suggestOverlayPruningCandidates`·`detectOverlayPolicyDrift`(compact-timeline 과다·overflow high without pruning·memory/knowledge scope 부재 등 휴리스틱, **warning only**). 진단 API에 **`overlayAssemblyPlanSummary`**·**`overlayPruningSummary`**·**`overlayPolicyDriftWarnings`** 노출(`overlayArchitecturePhase.current = "policy-guided-context-assembly-preparation-layer"`, `overlayMaturity.policyGuidedContextAssemblyPreparationLayer = true`, `overlayArchitecturePhase.autoPromptAssemblyEnabled = false`). **여전히 prompt 본문 자동 변경·자동 pruning 실행·retrieval/provider/orchestration 자율 실행 금지**.
7. **Policy-guided Prompt Assembly Layer** (다음 단계 준비) — **미도입**. controlled prompt assembly·deterministic context ordering·overflow mitigation·assembly tracing·memory/knowledge prioritization. **자율 execution·provider switching·retrieval rewriting·automatic blocking 금지 유지**.
8. **Runtime Policy Enforcement Layer** (향후) — **미도입** (hard gate·Cursor 차단·라우팅 강제 없음).
