...
export function buildDoclingProcessingEvidence(input: {
  ...
  return {
    ...
    normalization: {
      ...
      language: nd?.language ?? detail.versions[0]?.language ?? (isDocling ? snapshot.language : null),
      languageSource: nd?.languageSource ?? (detail.versions[0]?.language ? "PROVIDER" : null),
      title: nd?.title ?? null,
