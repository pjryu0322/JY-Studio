...
function validateCreatePackInput(input: ResolvedCreateProviderPackInput): string | null {
  ...
  if (!PACK_ID_PATTERN.test(packId)) {
    return "packId???ÅÎ¨∏ ?åÎ¨∏?? ?´Ïûê, ?òÏù¥?àÎßå 3~60?êÎ°ú ?ÖÎ†•??Ï£ºÏÑ∏??";
  }
  if (!categoryId) {
    return "Ïπ¥ÌÖåÍ≥†Î¶¨Í∞Ä ?ÑÏöî?©Îãà??";
  }
  if (name.length < 2 || name.length > 100) {
    return "?¥Î¶Ñ?Ä 2~100?êÎ°ú ?ÖÎ†•??Ï£ºÏÑ∏??";
  }
  if (shortDescription.length < 10 || shortDescription.length > 160) {
    return "ÏßßÏ? ?§Î™Ö?Ä 10~160?êÎ°ú ?ÖÎ†•??Ï£ºÏÑ∏??";
  }
  if (description.length < 20 || description.length > 1000) {
    return "?§Î™Ö?Ä 20~1000?êÎ°ú ?ÖÎ†•??Ï£ºÏÑ∏??";
  }
  if (tags.length > 10) {
    return "?úÍ∑∏??ÏµúÎ? 10Í∞úÍπåÏßÄ ?±Î°ù?????àÏäµ?àÎã§.";
  }
  if (!version) {
    return "Î≤ÑÏ†Ñ???ÑÏöî?©Îãà??";
  }
    ...
    data: {
      ...
      versions: {
        create: {
          ...
          targetUsers: [],
          useCases: [],
          versionSummary: `Ï¥àÏïà ${versionLabel}`,
        },
      },
    },
...
export async function updateProviderPackForClient(
  ...
  if (input.icon !== undefined) data.icon = input.icon.trim() || "?ì¶";
...
export async function createProviderPackVersionForClient(
  ...
  if (!version) {
    return { error: "VALIDATION" as const, message: "Î≤ÑÏ†Ñ???ÑÏöî?©Îãà??" };
  }
...
export async function createSourceDocumentForProviderPack(
  ...
  if (!title) {
    return { error: "VALIDATION" as const, message: "?úÎ™©???ÑÏöî?©Îãà??" };
  }
  if (!sourceType) {
    return { error: "VALIDATION" as const, message: "sourceType???ÑÏöî?©Îãà??" };
  }
  ...
  try {
    ...
    if ("runId" in run) {
      await completePipelineStep({
        ...
        message: `?êÏ≤ú Î¨∏ÏÑú ?±Î°ù (${sourceType})`,
        details: { validationStatus },
      });
      await finishPipelineRun({
        runId: run.runId,
        status: validationStatus === "WARNING" ? "WARNING" : "PASS",
        summary: "?êÏ≤ú Î¨∏ÏÑú ?±Î°ù Ï≤òÎ¶¨ ?ÑÎ£å",
      });
    } else {
  ...
  try {
    ...
    if ("runId" in run) {
      await completePipelineStep({
        ...
        status: "PASS",
        message: note ?? "Í≤Ä??Ï§ÄÎπ??ÑÎ£å",
      });
      await completePipelineStep({
        runId: run.runId,
        step: PipelineStatus.REVIEWING,
        status: "PASS",
        message: "Í¥ÄÎ¶¨Ïûê Í≤Ä???ÄÍ∏∞Ïó¥???±Î°ù",
      });
      await finishPipelineRun({
        runId: run.runId,
        status: "PASS",
        summary: "Í≤Ä???îÏ≤≠ ?úÏ∂ú ?ÑÎ£å",
      });
    } else {
...
export async function submitProviderPackForReview(userId: string, clientId: string, packId: string) {
  ...
  if (!pack.categoryId || !pack.shortDescription.trim() || !pack.description.trim()) {
    return { error: "INCOMPLETE" as const, message: "Ïπ¥ÌÖåÍ≥†Î¶¨?Ä ?§Î™Ö???ïÏù∏??Ï£ºÏÑ∏??" };
  }

  if (pack.versions.length === 0) {
    return { error: "INCOMPLETE" as const, message: "Î≤ÑÏ†Ñ??ÏµúÏÜå 1Í∞??ÑÏöî?©Îãà??" };
  }
  ...
  if (allDocs.length === 0) {
    return {
      ...
      message: "?êÏ≤ú Î¨∏ÏÑú(SourceDocument)Î•?ÏµúÏÜå 1Í∞??±Î°ù??Ï£ºÏÑ∏??",
    };
  }
  ...
  const submitNote = onlyEtc
    ? "Î™®Îì† ?êÏ≤ú Î¨∏ÏÑú ?†Ìòï??'Í∏∞Ì?(ETC)'?ÖÎãà?? ?êÎ£å ?†Ìòï??Íµ¨Ï≤¥?ÅÏúºÎ°?Î∂ÑÎ•ò?òÎ©¥ Í≤Ä???àÏßà???•ÏÉÅ?©Îãà??"
    : null;
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.packReview.update({
      ...
      data: {
        status: "WITHDRAWN",
        decision: "WITHDRAW",
        memo: "?úÍ≥µ?êÍ? Í≤Ä???îÏ≤≠???åÏàò?àÏäµ?àÎã§.",
        decidedAt: now,
      },
    });
  });
...
export async function evaluateProviderPackStructureQuality(userId: string, clientId: string, packId: string) {
  ...
  if ("error" in result) {
    ...
    return { error: "INCOMPLETE" as const, message: "Î≤ÑÏ†Ñ???ÜÏäµ?àÎã§." };
  }
