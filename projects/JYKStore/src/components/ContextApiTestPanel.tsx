      ...
      <form onSubmit={onSubmit} className="space-y-3">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            <h2 className="text-base font-bold text-slate-900">Context API 테스트</h2>
            <p className="mt-1 text-sm text-store-muted">
              <span className="font-semibold text-slate-800">{packName}</span>
              <span className="mx-1">·</span>
              <span className="font-mono text-xs text-slate-700">{packId}</span>
            </p>
          </div>
        ...
        <p className="text-xs text-store-muted">
          API Key는 이 화면에서만 사용되며 새로고침 시 사라집니다.
        </p>
        ...
        <div>
          ...
          <div className="mt-1.5 flex gap-2">
            <input
              ...
              className="min-h-[44px] min-w-0 flex-1 rounded-xl border border-store-border px-3 font-mono text-sm"
            />
            <button
              type="button"
              onClick={() => setShowKey((v) => !v)}
              className="inline-flex min-h-[44px] shrink-0 items-center rounded-xl border border-store-border px-3 text-xs font-semibold text-slate-700"
            >
              {showKey ? "숨기기" : "보이기"}
            </button>
          </div>
            ...
            <div className="mt-1.5 flex gap-2">
              {(["GET", "POST"] as const).map((m) => (
                <button
                  ...
                  type="button"
                  onClick={() => setMethod(m)}
                  className={`min-h-[44px] rounded-xl px-3 text-xs font-bold ${
                    method === m
        ...
        <div>
          ...
          <input
            ...
            className="mt-1.5 min-h-[44px] w-full rounded-xl border border-store-border px-3 text-sm"
          />
          <p className="mt-1.5 text-xs text-store-muted">
            query를 입력하면 title/content/section/tags/chunkType 기준으로 chunk ranking이 적용됩니다.
            includeMetadata=true일 때 응답 JSON에서 각 chunk의{" "}
            <span className="font-semibold">score</span>와{" "}
        ...
        <div className="flex flex-wrap gap-4">
          <div>
            ...
            <select
              ...
              className="mt-1.5 min-h-[44px] rounded-xl border border-store-border px-3 text-sm"
            >
              {[5, 10, 20, 50].map((n) => (
