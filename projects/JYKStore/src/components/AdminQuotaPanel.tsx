...
export function AdminQuotaPanel() {
  ...
  const refresh = useCallback(async (nextRange: QuotaSummaryRange) => {
    setLoading(true);
    setError(null);
    try {
      const data = await fetchAdminQuotaSummary({ range: nextRange });
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-store-muted">관리자 계정 세션으로 quota summary를 조회합니다.</p>
