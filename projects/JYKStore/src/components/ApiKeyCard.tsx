  ...
  return (
    <article
      className={`rounded-2xl border p-4 shadow-card ${
        revoked ? "border-slate-200 bg-slate-50 opacity-80" : "border-store-border bg-white"
      }`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <h2 className="text-sm font-bold text-slate-900">{apiKey.name}</h2>
          <p className="mt-1 font-mono text-xs text-slate-700">{apiKey.keyPrefix}••••••••</p>
          <p className="mt-2 text-[10px] text-store-muted">
            생성 {apiKey.createdAt}
            {apiKey.lastUsedAt ? ` · 마지막 사용 ${apiKey.lastUsedAt}` : null}
          </p>
        ...
        <span
          className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${
            revoked ? "bg-slate-200 text-slate-600" : "bg-green-50 text-green-700"
          }`}
        >
          {revoked ? "폐기됨" : "사용 중"}
        </span>
