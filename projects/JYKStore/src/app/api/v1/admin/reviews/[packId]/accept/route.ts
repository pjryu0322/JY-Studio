...
import { acceptPackReview } from "@/lib/admin-review-service";
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    const result = await acceptPackReview({
      packId: packId?.trim() ?? "",
    ...
    if (result.error === "NOT_REVIEWING") {
      return jsonWithClientIdCookie(
        { error: "검수 중(REVIEWING) 상태의 지식팩만 접수할 수 있습니다." },
        clientId,
        { status: 409 },
      );
    }
    ...
    logSafeRouteError({
      scope: "admin-review",
      method: "POST",
      path: "/api/v1/admin/reviews/[packId]/accept",
      error,
    });
