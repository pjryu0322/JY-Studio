...
/**
 * Restore PUBLISHED serving after unpublish via application transition.
 * Complements approvePackReview (REVIEWING → PUBLISHED) for the post-unpublish DRAFT case.
 */
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    if (result.error === "NOT_UNPUBLISHED_DRAFT") {
      return jsonWithClientIdCookie(
        { error: "게시 취소된(DRAFT) 지식팩만 재게시할 수 있습니다." },
        clientId,
        { status: 409 },
      );
    }
