/**
 * PATCH /api/v1/admin/packs/[packId]/knowledge-scope/items/[itemId] — single decision
 */
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const item = await updateInventoryItemDecision({
    ...
    return jsonWithClientIdCookie({ clientId, item }, clientId, { status: 200 });
  } catch (error) {
    return mapKnowledgeScopeInventoryError(
      error,
      clientId,
      "admin/knowledge-scope/items/[itemId]",
      "PATCH",
      `/api/v1/admin/packs/${packId}/knowledge-scope/items/${itemId}`,
    );
  }
}
