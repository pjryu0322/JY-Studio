/**
 * POST /api/v1/admin/packs/[packId]/correction/cases/[caseId]/verify
 */
import { NextRequest } from "next/server";
import { requireAdminSession } from "@/lib/admin-route-guard";
import { ensureClientId, jsonWithClientIdCookie } from "@/lib/client-identity";
import { verifyCorrectionCase } from "@/lib/correction/correction-lifecycle-service";
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    const correctionCase = await verifyCorrectionCase({
      packId: packId?.trim() ?? "",
      caseId: caseId?.trim() ?? "",
    ...
    return mapCorrectionRouteError(
      error,
      clientId,
      "admin/correction/verify",
      "POST",
      `/api/v1/admin/packs/${packId}/correction/cases/${caseId}/verify`,
    );
  }
}
