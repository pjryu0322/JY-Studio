...
import { retryDoclingImportByBundleId } from "@/lib/docling-import/docling-import-service";
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    const result = await retryDoclingImportByBundleId({
      userId,
      clientId,
      packId: packId?.trim() ?? "",
    ...
    logSafeRouteError({
      ...
      method: "POST",
      path: "/api/v1/provider/packs/[packId]/docling-import/[bundleId]/retry",
      error,
    });
