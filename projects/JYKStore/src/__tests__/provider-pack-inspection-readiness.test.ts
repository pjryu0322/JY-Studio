...
describe("provider pack inspection readiness", () => {
  it("Case 3: 0/4 shows structure quality start as next action", () => {
    ...
    assert.equal(readiness.nextActionLabel, "구조/품질 점검 시작");
    assert.equal(readiness.completedCount, 0);
    assert.equal(readiness.totalCount, 4);
    assert.equal(readiness.canSubmitReview, false);
  });

  it("Case 4: later steps wait until structure is done", () => {
    const readiness = buildProviderInspectionReadiness({
      pack: basePack(),
      ...readyInput,
    });

    assert.equal(readiness.steps[0]?.status, "ready");
  ...
  it("Case 5: all checks done → go to submit review", () => {
    const readiness = buildProviderInspectionReadiness({
      pack: basePack({
    ...
    assert.equal(readiness.nextActionLabel, "검수요청으로 이동");
