import {
  toSafeLogError as sharedToSafeLogError,
  type SafeLogError,
} from "../src/lib/safe-logging";

export type { SafeLogError };
export { sanitizeLogMessage } from "../src/lib/safe-logging";
...
/** MCP-facing wrapper: preserve MCP default error code when code is absent. */
export function toSafeLogError(error: unknown): SafeLogError {
  return sharedToSafeLogError(error, {
    defaultCode: "JYKSTORE_MCP_INTERNAL_ERROR",
    defaultMessage: "Unexpected MCP error.",
  });
}
