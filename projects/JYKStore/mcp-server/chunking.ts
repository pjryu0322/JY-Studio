...
function assertValidByteOffset(buffer: Buffer, offset: number): void {
  if (offset < 0 || !Number.isInteger(offset)) {
    throw mcpError(
      "JYKSTORE_MCP_INVALID_CHUNK_RANGE",
      "offset must be a non-negative integer byte offset.",
    );
  }
  ...
  if (isUtf8Continuation(buffer[offset]!)) {
    throw mcpError(
      ...
      {
        hint: "Use a nextOffset returned by a previous chunk read.",
      },
    );
  }
}
...
export function sliceUtf8TextByBytes(
  ...
  if (!Number.isInteger(limitBytes) || limitBytes < 1) {
    throw mcpError(
      "JYKSTORE_MCP_INVALID_CHUNK_RANGE",
      "limitBytes must be a positive integer.",
    );
  }
